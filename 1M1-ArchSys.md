...
## Récolte de données du Micro:MicroBit @fullscreen

Pour récolter des données sur l'ordinateur, il faut que le programme du MicroBit envoit des informations sur le port série **USB**.
...
> ℹ️ La variable prédéfinie pour la température ``||Entrée:température (° C)||``
  et celle pour la luminosité ``||Entrée:niveau d'intensité lumineuse||`` se trouvent dans la catégorie **Entrée**.

L'exemple suivant montre comment la **température** et la **luminosité** sont envoyées chaque seconde
sur le **port série** (**USB**) afin de pouvoir les **visualiser** et les **sauvegarder** sur l'ordinateur.

> 💡 La **température** et la **luminosité** proviennent de deux **variables prédéfinies** dans **Makecode**
  et elles contiennent les **valeurs mesurées** par les **capteurs internes** correspondants du Micro:Bit (Sonde de température du CPU, matrice de leds)
...
> ⚠️ Il est important de télécharger le programme sur le MicroBit à chaque **modification**.
